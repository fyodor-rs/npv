...
// login, logout, exit, newTerminal, newSftp, newRemoteDesktop, openControlPanel
...
//
// Copyright (C) 2013-2022 by Bitvise Limited.
//
	...
	try
	{
		...
		if (argc != 3)
		{
			cout << "Bitvise SSH Client Control Utility" << endl
				 << "Copyright (C) 2012-2022 by Bitvise Limited" << endl
				 << endl
				 << "Usage: BvSshCtrl ProcessID Command" << endl
				 << endl
				 << "ProcessID   Bitvise SSH Client's process ID" << endl
				 << "Command     Currently supported commands: Login, Logout, Exit, NewTerminal," << endl
				 << "            NewSftp, NewRemoteDesktop, OpenControlPanel" << endl;
